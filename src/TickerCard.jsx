...
const TickerCard = ({ ticker }) => {
        ...
        return (
            <div className="bg-white rounded-lg shadow-lg p-6 w-80 mx-auto animate-pulse">
                <div className="h-6 bg-gray-200 rounded w-3/4 mb-4">
                    <div className="text-sm font-semibold text-gray-500">{ticker} 로딩중...</div>
                </div>
    ...
    const stockUrl = isKoreanStock ? `https://finance.naver.com/item/main.naver?code=${meta.symbol.replace('.KS', '').replace('.KQ', '')}` : `https://www.google.com/finance/quote/${ticker}:${meta.exchangeName == 'NMS' ? "NASDAQ" : "NYSE"}`;
    ...
    return (
        <a href={stockUrl} target="_blank" rel="noopener noreferrer">
            <div className="bg-white rounded-lg shadow-xl p-6 w-80 transform transition duration-500 hover:scale-105">
                ...
                <div className={`text-base font-semibold {currencyMarker}{isPositive ? 'text-green-700' : 'text-red-700'}`}>
